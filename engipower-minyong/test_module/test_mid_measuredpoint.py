"""
    @File: test_mid_measuredpoint.py
    @Description: 描述
    @Author: wangk
    @UPDATED BY: wangk
    @UPDATED: 2021/06/02
...
data = MidHandler.excel.read('中间层-表计')
...
@pytest.mark.mid_measurepoint
@pytest.mark.parametrize("test_data", data)
def test_mid_measuredpoint(test_data, login):
    """
    小井监测-接口
    ...
    req_params = test_data['url_param']
    ...
    try:
        ...
        if expected['mode'] == 'realtime':
            ...
            # 断言接口数据类型
            assert p_type + '_m_' + req_data_json['type'] == demo_data_dict['type']

            # 补充断言
            # 查询列是否存在返回结果中
            assert req_data_json['foundColumn'] in demo_data_dict
            # 返回结果数量小于limit
            assert len(resp_body) < request_p_data_dict['limit']
